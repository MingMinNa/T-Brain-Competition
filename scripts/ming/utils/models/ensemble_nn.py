...
class EnssembleRegressionModel(nn.Module):
    def __init__(self, input_dim, input_features):
        super(EnssembleRegressionModel, self).__init__()
...
def build_model(X_train, y_train, input_features,
    ...
    model = EnssembleRegressionModel(input_dim, input_features).to(device)
    ...
    best_model = EnssembleRegressionModel(X_train.shape[1], input_features).to(device)
